...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	for (int i = 0; i < num_neigh; i++) {
		if (neigh[i] != v[rank]) {
			MPI_Recv(vRecv, numProcs, MPI_INT, neigh[i], 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	...
	*/
	...
	if (rank != leader) {
		...
		MPI_Recv(v, numProcs, MPI_INT, v[rank], 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; i++) {
			...
			MPI_Recv(&q, 1, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; i++) {
			...
			MPI_Recv(&recvd, 1, MPI_DOUBLE, neigh[i], 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
	...
	for (int i = 0; i < nProcesses; i++) {
		if (parents[i] == rank) {
			for (int j = 0; j < nProcesses; j++) {
				MPI_Recv(vTopology[j], nProcesses, MPI_INT, i, 4 + j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

				for (int k = 0; k < nProcesses; k++) {
					if (vTopology[j][k] == 1 && topology[j][k] == 0) {
	...
	if (rank != leader) {
		...
		for (int i = 0; i < nProcesses; i++) {
			MPI_Send(topology[i], nProcesses, MPI_INT, parents[rank], 4 + i, MPI_COMM_WORLD);
		}
		
		/* TODO4: Daca nu suntem liderul, asteptam topologia completa de la parinte  */
		for (int i = 0; i < nProcesses; i++) {
			MPI_Recv(topology[i], nProcesses, MPI_INT, parents[rank], 4 + i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	}
	...
	for (int i = 0; i < nProcesses; i++) {
		if (parents[i] == rank) {
			for (int j = 0; j < nProcesses; j++) {
				MPI_Send(topology[j], nProcesses, MPI_INT, i, 4 + j, MPI_COMM_WORLD);
			}
		}
	}
